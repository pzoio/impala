...
public class PluginContextHolder extends SpringContextHolder {
	...
	public void loadParentContext(Object test, PluginSpec pluginSpec) {
		...
		if (classLoader instanceof TestClassLoader) {
			TestClassLoader tcl = (TestClassLoader) classLoader;
			classLoader = tcl.getParent();
		}
		loadParentContext(classLoader, pluginSpec);
	}

	public boolean loadParentContext(ClassLoader classLoader) {
		if (currentPluginSpec == null) {
			throw new IllegalStateException("No plugin spec currently define. You may not have run a test.");
		}
