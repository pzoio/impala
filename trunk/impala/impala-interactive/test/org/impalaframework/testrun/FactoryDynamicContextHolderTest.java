...
public class FactoryDynamicContextHolderTest extends TestCase {
	...
	public void tearDown() {
		...
		try {
			FactoryDynamicContextHolder.remove(ParentSpec.NAME);
		}
		catch (Exception e) {
		}
	}
	...
	public void testNoInit() {
		try {
			FactoryDynamicContextHolder.get();
			fail();
		}
	...
	public void testInit() {
		...
		PluginStateManager holder = FactoryDynamicContextHolder.getPluginStateManager();

		final Test1 test1 = new Test1();
		FactoryDynamicContextHolder.init(test1);
		...
		FileMonitor pluginBean = FactoryDynamicContextHolder.getPluginBean(test1, plugin1, "bean1", FileMonitor.class);
		assertEquals("classes.FileMonitorBean1", pluginBean.getClass().getName());
		
		try {
			FactoryDynamicContextHolder.getPluginBean(test1, "unknown-plugin", "bean1", FileMonitor.class);
			fail();
		}
		catch (NoServiceException e) {
		...
		FactoryDynamicContextHolder.init(test2);
		...
		FactoryDynamicContextHolder.init(test3);
		...
		FileMonitor f3PluginBean = FactoryDynamicContextHolder.getPluginBean(test3, plugin1, "bean3", FileMonitor.class);
		...
		assertTrue(FactoryDynamicContextHolder.hasPlugin(plugin1));
		assertTrue(FactoryDynamicContextHolder.hasPlugin(plugin2));
		assertTrue(FactoryDynamicContextHolder.hasPlugin(plugin3));

		// show that this will return false
		assertFalse(FactoryDynamicContextHolder.reload(test3, "unknown"));

		// now reload plugin1
		assertTrue(FactoryDynamicContextHolder.reload(test3, plugin1));
		assertTrue(holder.hasPlugin(plugin1));
		...
		assertTrue(FactoryDynamicContextHolder.reload(test3, plugin2));
		assertTrue(holder.hasPlugin(plugin2));
		...
		assertNull(FactoryDynamicContextHolder.reloadLike(test3, "unknown"));

		// now test reloadLike
		assertEquals(plugin2, FactoryDynamicContextHolder.reloadLike(test3, "plugin2"));
		f3reloaded = (FileMonitor) context3.getBean("bean3");
		service(f3reloaded);

		// now remove plugin2 (and by implication, child plugin3)
		assertFalse(FactoryDynamicContextHolder.remove("unknown"));
		assertTrue(FactoryDynamicContextHolder.remove(plugin2));
	...
	public void testAdd() {
		...
		final ApplicationContext context1 = FactoryDynamicContextHolder.get();
		FileMonitor f1 = (FileMonitor) context1.getBean("bean1");
		FileMonitor f2 = (FileMonitor) context1.getBean("bean2");

		service(f1);
		noService(f2);
		FactoryDynamicContextHolder.addPlugin(new SimplePluginSpec(plugin2));
		service(f1);
		service(f2);
	}
	...
	public void testReloadParent() {
		...
		final ApplicationContext context1a = FactoryDynamicContextHolder.get();
		FileMonitor f1 = FactoryDynamicContextHolder.getBean(test1, "bean1", FileMonitor.class);
		service(f1);
		FactoryDynamicContextHolder.reloadParent();
		final ApplicationContext context1b = FactoryDynamicContextHolder.get();
		f1 = FactoryDynamicContextHolder.getBean(test1, "bean1", FileMonitor.class);
		service(f1);
	...
	public void testUnloadParent() {
		final Test1 test1 = new Test1();
		FactoryDynamicContextHolder.init(test1);
		FactoryDynamicContextHolder.unloadParent();
		try {
			FactoryDynamicContextHolder.get();
		}
		catch (NoServiceException e) {
