...
public class BeanSetBeanDefinitionDocumentReader extends DefaultBeanDefinitionDocumentReader {
	...
	private static final Log log = LogFactory.getLog(BeanSetBeanDefinitionDocumentReader.class);

	public BeanSetBeanDefinitionDocumentReader() {
		super();
		importedResources = BeanSetImportDelegate.getImportedResources().get();
		namedResources = BeanSetImportDelegate.getNamedResources().get();
		beanSetMappings = BeanSetImportDelegate.getBeanSetMappings().get();
	}
	...
	public void registerBeanDefinitions(Document doc, XmlReaderContext readerContext) {
		...
		if (log.isInfoEnabled())
			log.info("Registering definitions for " + filename);
	...
	protected void importBeanDefinitionResource(Element element) {
		...
		if (log.isDebugEnabled())
			log.debug("Handling import definition location ... " + location);
		...
		if (location.startsWith(BEANSET_PREFIX)) {
			...
			if (log.isDebugEnabled())
				log.debug("Beanset key ... " + beanSetName);
		...
		if (!ignore) {
			if (log.isInfoEnabled())
				log.info("Importing resource " + location);
	...
	void notifyAlreadyLoaded(String location) {
		if (log.isInfoEnabled()) {
			log.info("Ignoring resource loaded explicitly: " + location);
		}
	}

	void notifyAlreadyImported(String location) {
		if (log.isInfoEnabled()) {
			log.info("Ignoring resource already imported: " + location);
		}
	}
